...
# """
# Adaptation of 
# https://github.com/scieloorg/articles_meta
#     articles_meta/articlemeta/export_rsps.py
# """
# # import re
...
def get_xml_rsps(document, journal=None, issue=None):
    """
    ...
    """
    # data = {}
    # data["article"] = document.records
    # data["issue"] = issue.record
    # data["journal"] = journal.record
    # data["body"] = document.get_body(website_url)
...
class XMLArticleAttributesPipe(plumber.Pipe):
    ...
    def transform(self, data):
        ...
        xml.set('{http://www.w3.org/XML/1998/namespace}lang', raw.language)
        xml.set(
            'article-type',
            translate_document_type.get(
                raw.document_type, raw.document_type))
...
class XMLJournalMetaPublisherPipe(plumber.Pipe):
    ...
    def transform(self, data):
        ...
        publishername.text = raw.journal.get_publisher_names()

        publisherloc = ET.Element('publisher-loc')
        publisherloc.text = raw.journal.get_publisher_loc()

        if publishername.text:
            publisher.append(publishername)
        if publisherloc.text:
...
class XMLArticleMetaArticleIdPublisherPipe(plumber.Pipe):
    ...
    def transform(self, data):
        ...
        ids = (
            (raw.order, 'other', ''),
            (raw.ahead_of_print_pid, 'publisher-id', 'previous-pid'),
            (raw.scielo_pid_v1, 'publisher-id', 'scielo-v1'),
            (raw.scielo_pid_v2, 'publisher-id', 'scielo-v2'),
            (raw.scielo_pid_v3, 'publisher-id', 'scielo-v3'),
        )

        for value, publisher_id, specific_use in ids:
            if value:
                article_id = ET.Element('article-id')
                article_id.text = value
                article_id.set('pub-id-type', publisher_id)
                if specific_use:
...
class XMLArticleMetaTranslatedTitleGroupPipe(plumber.Pipe):
    ...
    def transform(self, data):
        ...
        html_fulltext_langs = []
        if raw.translated_html_body_items:
            html_fulltext_langs = [
                i
                for i in raw.translated_html_body_items.keys()]
...
class XMLArticleMetaContribGroupPipe(plumber.Pipe):
    ...
    def transform(self, data):
        ...
        for author in raw.contrib_group:
            ...
            for xref_type, xref_rid in author.get('xref', []):
                xref = ET.Element('xref')
                xref.set('ref-type', xref_type)
